...
from config import db_config, etl_strategies

def get_api_data(url):
  r = requests.get(url)
  return r

def process_etl(db_config, etls_config):
  def build_filename(filename, suffix):
    [name, ext] = split('\\.', filename)
    output_filename = f'{name}{suffix}.{ext}'
    return output_filename

  def process_etl_strategies(db_cfg, strategy, file_suffix):
  ...
  try:
    ...
    for etl_strategy in etls_config:
      process_etl_strategies(db_config, etl_strategy, file_suffix)
...
process_etl(db_config, etl_strategies)
