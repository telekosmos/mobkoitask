...
@mock.patch("builtins.open", new_callable=mock.mock_open)
@mock.patch("os.makedirs")
def test_malformed_url(mock_makedirs, mock_file, mock_requests, mock_pg_connect):
  ...
  try:
    ...
    assert mock_pg_connect.call_count == 0

@mock.patch("psycopg2.connect")
@mock.patch("requests.get")
@mock.patch("builtins.open", new_callable=mock.mock_open)
@mock.patch("os.makedirs")
def test_db_no_connection(mock_makedirs, mock_file, mock_requests, mock_pg_connect):
  mock_pg_connect.side_effect = OperationalError()
  ...
  """
  mock_requests.return_value = MockResponse(200, binance_tickers_resp_fixture)
  strategies = [etl_binance_config]
  try:
    process_etl(db_config, strategies)
  finally:
    assert mock_requests.call_count == 1
    assert mock_pg_connect.call_count == 3 # retries
    assert mock_makedirs.call_count == 0
    assert mock_file.call_count == 0
